...
class Qwen3DiskCache(DynamicCache, oCache):
    ...
    def update(
        ...
        self.key_cache[layer_idx], self.value_cache[layer_idx] = torch.empty(
            0), torch.empty(0)
        return out
...
class MyQwen3Model(Qwen3Model):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Offload embeddings during layer processing
        ...
        if stats:
            print("./qwen3.forward.", datetime.now().strftime("%H:%M:%S"),
...
modeling.Qwen3DecoderLayer = MyQwen3DecoderLayer
...
class MyQwen3ForCausalLM(Qwen3ForCausalLM):
    ...
    def offload_layers_to_gpu_cpu(self, gpu_layers_num=0, cpu_layers_num=0):
        ...
        layer_idx = 0
        while (gpu_layers_num > 0 or cpu_layers_num >
               0) and layer_idx < self.num_hidden_layers:
